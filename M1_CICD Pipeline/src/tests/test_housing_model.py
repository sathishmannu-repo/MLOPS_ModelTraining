...
def create_sample_data(n_samples=100):
    ...
    y = (
        3 * X[:, 0] + 5 * X[:, 1] +
        np.random.normal(0, 0.1, n_samples)  # Target variable
    )
    return X, y
...
def test_housing_model():
    ...
    )  # Assuming housing prices cannot be negative.
...
def test_model_training():
    ...
    assert hasattr(model.model, 'coef_'), (
        "Model coefficients should be set after training."
    )
    assert hasattr(model.model, 'intercept_'), (
        "Model intercept should be set after training."
    )
